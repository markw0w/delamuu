...
function OrdersHistory() {
  ...
  const handleFilter = async () => {
    if (!filterDate) return; // Si no se ha seleccionado una fecha, no filtrar
    try {
      // Usamos params para enviar la fecha como query parameter
      const response = await axios.get(API_URL_FILTER_ORDERS, {
        params: { date: filterDate },
      });
      setFilteredOrders(response.data);
      setCurrentPage(1); // Reiniciamos la página al aplicar el filtro
    } catch (error) {
      console.error("Error al filtrar las órdenes:", error);
    }
  };

  // Si se ha aplicado un filtro, usamos esas órdenes; sino, todas
...
function OrdersHistory() {
  ...
  const filterOrders = async () => {
    try {
      ...
      console.error("Error al filtrar las fechas:", error);
    }
  };
